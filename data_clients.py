...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    def predict_interaction(x_var, y_var, z_var, w_var, h_var, model_var, results, wval, hval, coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, obj_name):
                ...
                if view_img:
                    ...
                    for obj_id in id_list:
                        ...
                        if obj_id == id_list[1] and True:  #cellphone 使用しない場合はTrueをFalseに
                            predict_interaction(x,y,z,w,h,model_cellphone,results, wval,hval,coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, 'cellphone')
                        if obj_id == id_list[2] and True:  #book 使用しない場合はTrueをFalseに
                            predict_interaction(x,y,z,w,h,model_book,results, wval,hval,coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, 'book')
                        if obj_id == id_list[3] and True:  #keyboard 使用しない場合はTrueをFalseに
                            predict_interaction(x,y,z,w,h,model_keyboard,results, wval,hval,coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, 'keyboard')
                        if obj_id == id_list[4] and True:  #bottle 使用しない場合はTrueをFalseに
                            predict_interaction(x,y,z,w,h,model_bottle,results, wval,hval,coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, 'bottle')
                        if obj_id == id_list[5] and True:  #cushion 使用しない場合はTrueをFalseに
                            predict_interaction(x,y,z,w,h,model_cushion,results, wval,hval,coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, 'cushion')
                        if obj_id == id_list[6] and True:  #food 使用しない場合はTrueをFalseに
                            predict_interaction(x,y,z,w,h,model_food,results, wval,hval,coords_s_pre, distance_s, interaction_class_out, interaction_prob_out, 'food')
