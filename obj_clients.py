    ...
    for path, depth, distance, depth_scale, img, im0s, vid_cap, color_intr in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        cx = min(wval-1, round(x+w/2))
                        cx = max(0, cx)
                        cy = min(hval-1, round(y+h/2))
                        cy = max(0, cy)
            ...
            if view_img:
                ...
                if k == 13:
                    print(box_list)
                    for obj_loc in box_list:
                        #マップ生成サーバーに座標を送信
                        print(float(obj_loc[7]), float(obj_loc[3]))
                        point = Point(float(obj_loc[7]), float(obj_loc[3]))
                        #add_probabilityでobjectごとのインタラクションとその確率を追加
                        #座標系変換
                        point.convert_location(X,Z,THETA,PITCH)
                        data_dict = add_location_data(data_dict, names[int(obj_loc[0])], point)
                    print(data_dict)
                    if CONNECT:
